...
<template>
  <div class="texture-inspector flex flex-col select-none text-xs font-sans" @dragover.prevent="dragging = true" @dragleave.prevent="dragging = false" @drop.prevent="dropFile">
    ...
    <header class="inspector-head">
      ...
      <span class="inspector-head-name">{{ activeTexture?.name || 'No image' }}</span>
    </header>
    <div class="px-2.5 py-1.5 border-b border-ui-borderSubtle">
      <nav class="inspector-seg is-stretch" aria-label="Texture inspector sections">
        <button
          v-for="item in (['image', 'library', 'advanced'] as const)"
          :key="item"
          type="button"
          ...
          @click="tab = item"
        >
          {{ item === 'image' ? 'Image' : item === 'library' ? 'Library' : 'Advanced' }}
        </button>
      </nav>
    </div>
    ...
    <template v-if="tab === 'image' && activeTexture">
      <section class="texture-card">
        ...
        <div class="texture-summary">
          <strong :title="activeTexture.name">{{ activeTexture.name }}</strong>
          <span>{{ activeTexture.width }} × {{ activeTexture.height }} px</span>
          <span>{{ layerCount }} {{ layerCount === 1 ? 'layer' : 'layers' }} · {{ usedBy.length ? `${usedBy.length} ${usedBy.length === 1 ? 'object uses' : 'objects use'} this` : 'Not applied to an object' }}</span>
        </div>
      </section>
      <div class="texture-actions">
        <UiButton size="xs" @click="showNew = true">New</UiButton>
      ...
      <div class="texture-actions">
        <UiButton size="xs" variant="accent" @click="openEditor('paint')">Open Paint</UiButton>
        <UiButton size="xs" @click="openEditor('uv')">Edit UV layout</UiButton>
        <UiButton size="xs" @click="exportPng">Export PNG</UiButton>
      </div>
      <div class="texture-actions">
        <UiButton size="xs" @click="openTileset(projectStore.activeTextureId)" title="Browse and edit tiles on this atlas">Tileset</UiButton>
        <UiButton size="xs" @click="projectStore.duplicateTexture(activeTexture.id)">Duplicate</UiButton>
      </div>
      <UiSection title="Use on object" blender-icon="mesh-cube" :default-open="true">
        <div class="texture-assignment">
          <strong>{{ projectStore.activeMesh?.name || 'No object selected' }}</strong>
          <span :class="{ assigned }">{{ assigned ? 'Using this image' : 'Not using this image' }}</span>
        </div>
        <UiButton
          v-if="!assigned"
          size="xs"
          ...
          @click="applyToActiveMesh(activeTexture.id)"
        >
          Apply to selected object
        </UiButton>
        <p v-if="usedBy.length > 1">Painting updates all {{ usedBy.length }} objects. Duplicate to paint a separate copy.</p>
      </UiSection>
      <UiSection title="Image settings" blender-icon="texture" :default-open="true">
        ...
        <details class="texture-details">
          <summary>Resize image or canvas <span>{{ activeTexture.width }} × {{ activeTexture.height }}</span></summary>
          <div class="texture-details-body">
            ...
            <select v-model="resizeMode" class="inspector-select w-full" aria-label="Texture resize method">
              <option value="resample">Scale pixels (nearest neighbor)</option>
              <option value="crop">Change canvas (keep pixel size)</option>
            </select>
            <UiButton
              size="xs"
              :title="resizeMode === 'crop' ? 'Adds or removes space at the right and bottom edges.' : 'Scales every layer to the new dimensions with crisp pixels.'"
              @click="resize"
            >
              {{ resizeMode === 'crop' ? 'Change canvas size' : 'Resize image' }}
            </UiButton>
          </div>
        </details>
        <details class="texture-details">
          <summary>Remove or reset…</summary>
          <div class="texture-details-body">
    ...
    <UiSection v-else-if="tab === 'library'" title="Library" blender-icon="texture" :default-open="true">
      <div class="texture-actions">
        <UiButton size="xs" variant="accent" @click="showNew = true">+ New image</UiButton>
        <UiButton size="xs" @click="fileInput?.click()">Import image</UiButton>
      </div>
      <input class="texture-search inspector-select w-full" v-model="query" type="search" aria-label="Search texture images" placeholder="Search images…" />
    ...
    <UiSection v-else-if="tab === 'advanced' && activeTexture" title="Texture atlas" blender-icon="grid" :default-open="true">
      <UiButton size="xs" variant="accent" @click="openTileset(projectStore.activeTextureId)">Open tileset editor</UiButton>
      <details class="texture-details">
        <summary>Tile grid <span>{{ activeTexture.atlas ? `${activeTexture.atlas.cols} × ${activeTexture.atlas.rows}` : 'Off' }}</span></summary>
        <div class="texture-details-body">
          <label>Columns<input type="number" min="1" max="16" v-model.number="cols" /></label>
          <label>Rows<input type="number" min="1" max="16" v-model.number="rows" /></label>
          <div class="texture-actions">
            <UiButton size="xs" @click="setGrid">Apply grid</UiButton>
            <UiButton size="xs" :disabled="!activeTexture.atlas" @click="projectStore.clearTextureAtlasGrid(activeTexture.id)">Remove grid</UiButton>
          </div>
          <UiButton size="xs" @click="openEditor('uv')">Open UV editor</UiButton>
      ...
      <details class="texture-details">
        <summary>Scene-wide tools</summary>
        <div class="texture-details-body">
          <UiButton size="xs" @click="applyEverywhere">Apply image to all materials…</UiButton>
          <UiButton size="xs" @click="bakeAtlas">Combine scene textures into atlas…</UiButton>
        </div>
      </details>
    </UiSection>
    <div v-else class="px-2.5 py-2 space-y-1.5">
      <p>Create or import an image to begin.</p>
      <UiButton size="xs" @click="showNew = true">New image</UiButton>
      <UiButton size="xs" @click="fileInput?.click()">Import image</UiButton>
    </div>
...
<style scoped>
.texture-inspector { color: var(--ui-text-secondary); position: relative; }
.checker { background: repeating-conic-gradient(#35363c 0% 25%, #222329 0% 50%) 0 0 / 8px 8px; image-rendering: pixelated; }
.texture-card { display: flex; gap: 8px; padding: 8px 10px; align-items: center; border-bottom: 1px solid var(--ui-border-subtle); }
.texture-preview { width: 56px; height: 56px; padding: 3px; border: 1px solid var(--ui-border-subtle); border-radius: 3px; flex-shrink: 0; }
.texture-preview img { width: 100%; height: 100%; object-fit: contain; image-rendering: pixelated; }
.texture-summary { display: flex; flex-direction: column; min-width: 0; gap: 3px; font-size: 10px; color: var(--ui-text-muted); }
...
.texture-actions { display: flex; gap: 4px; margin: 0 10px 6px; }
.texture-actions > * { flex: 1; min-width: 0; }
p { color: var(--ui-text-muted); font-size: 10px; line-height: 1.45; margin: 0; }
.texture-assignment { display: flex; flex-direction: column; gap: 2px; background: var(--ui-bg-input); padding: 6px 8px; border-radius: 3px; }
.texture-assignment strong { color: var(--ui-text-primary); }
.texture-assignment span { font-size: 10px; }
.texture-assignment .assigned { color: var(--ui-text-accent); }
label { display: flex; align-items: center; justify-content: space-between; gap: 8px; }
input, select { min-width: 0; background: var(--ui-bg-input); color: var(--ui-text-primary); border: 1px solid var(--ui-border-subtle); border-radius: 3px; padding: 4px 6px; font: inherit; }
label input { width: 65%; }
input[type=number] { width: 80px; }
.texture-details { border: 1px solid var(--ui-border-subtle); border-radius: 3px; }
summary { padding: 6px 8px; cursor: pointer; font-size: 10px; }
summary span { color: var(--ui-text-muted); float: right; }
.texture-details-body { padding: 4px 8px 8px; display: flex; flex-direction: column; gap: 6px; }
.texture-search { width: 100%; height: 22px; }
.texture-library { display: flex; flex-direction: column; gap: 5px; }
.texture-library-row { display: flex; align-items: center; gap: 8px; text-align: left; border: 1px solid var(--ui-border-subtle); padding: 6px; border-radius: 3px; min-width: 0; cursor: pointer; }
.texture-library-row.active { background: var(--ui-bg-active); border-color: var(--ui-border-strong); }
.texture-library-row img { width: 38px; height: 38px; object-fit: contain; flex-shrink: 0; }
.texture-library-row > span { min-width: 0; }
.texture-library-row strong { display: block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 10px; color: var(--ui-text-primary); }
.texture-library-row small { color: var(--ui-text-muted); font-size: 9px; }
.texture-selected { margin-left: auto; font-size: 9px; color: var(--ui-text-accent); }
