    ...
    <div class="fixed inset-0 z-[120] flex items-center justify-center bg-black/65 p-3 backdrop-blur-sm" @keydown="keys">
      <section ref="panel" role="dialog" aria-modal="true" aria-labelledby="humanoid-title" tabindex="-1" class="flex h-[min(820px,94vh)] w-[min(1200px,96vw)] min-h-0 flex-col overflow-hidden rounded-xl border border-ui-borderStrong bg-ui-panel text-ui-textPrimary shadow-2xl outline-none">
        <header class="flex shrink-0 items-center justify-between border-b border-ui-borderSubtle bg-ui-header px-6 py-4">
          <div class="flex items-center gap-3"><PersonStanding class="h-5 w-5 text-ui-textMuted" /><div><h2 id="humanoid-title" class="text-sm font-semibold">Humanoid auto-rigger</h2><p class="mt-0.5 text-[11px] text-ui-textMuted">Fit to your character. Preview before applying.</p></div></div>
        ...
        <div class="grid min-h-0 flex-1 grid-cols-[minmax(0,1fr)_320px] max-[760px]:grid-cols-[minmax(0,1fr)_260px]">
          <div class="relative min-h-0 border-r border-ui-borderSubtle">
            ...
            <div v-if="draft" class="absolute right-4 top-4 inspector-seg"><button v-for="option in (['front', 'side', 'perspective'] as const)" :key="option" type="button" class="inspector-seg-btn" :class="{ 'is-active': view === option }" @click="view = option">{{ option === 'side' ? 'Side' : option === 'front' ? 'Front' : '3D' }}</button></div>
          </div>
          <aside class="min-h-0 overflow-y-auto p-5 text-xs custom-scrollbar space-y-5">
            <template v-if="stage === 0">
              ...
              <label class="block space-y-2"><span>Model</span><select v-model="meshId" aria-label="Humanoid model" class="inspector-select w-full" @change="loadModel"><option value="" disabled>Choose a model</option><option v-for="mesh in project.meshes" :key="mesh.id" :value="mesh.id">{{ mesh.name }}</option></select></label>
              <label class="block space-y-2"><span>Starting pose</span><select v-model="pose" aria-label="Starting pose" class="inspector-select w-full" @change="resetMarkers"><option value="t">T-pose · arms out</option><option value="a">A-pose · arms lowered</option></select></label>
            ...
            <template v-else-if="stage === 1">
              ...
              <div class="space-y-1"><button v-for="item in landmarkGroups" :key="item.id" type="button" class="flex w-full items-center gap-3 rounded border px-3 py-1.5 text-left" :class="group === item.id ? 'border-ui-borderStrong bg-ui-active' : 'border-transparent hover:bg-ui-hover'" @click="group = item.id"><span class="h-4 w-4 rounded-full border-[3px]" :style="{ borderColor: item.color }" /><span class="flex-1">{{ item.label }}</span><Check v-if="confirmed.includes(item.id)" class="h-3.5 w-3.5 text-ui-textPrimary" /></button></div>
              ...
              <label class="block space-y-2"><span>Skeleton detail</span><select v-model="detail" aria-label="Skeleton detail" class="inspector-select w-full"><option value="standard">Standard · 19 bones</option><option value="simple">Simple · 15 bones</option></select></label>
            ...
            <template v-else>
              ...
              <p class="rounded border border-ui-borderSubtle bg-ui-surface p-3 text-ui-textSecondary">{{ previewBones.length }} bones · {{ status.ready ? 'Binding checks passed' : 'Check binding before applying' }}</p>
              <label class="block space-y-2"><span>Test joint</span><select v-model="testBone" aria-label="Preview test joint" class="inspector-select w-full" @change="applyBend"><option v-for="bone in previewBones" :key="bone.id" :value="bone.name">{{ bone.name }}</option></select></label>
              <label class="block space-y-2"><span>Bend · <span class="inspector-value">{{ bend }}°</span></span><input v-model.number="bend" aria-label="Preview bend" type="range" min="-75" max="75" step="1" class="inspector-range" @input="applyBend" /></label>
            ...
            <template v-if="stage === 1"><label class="block space-y-2"><span>Weighting method</span><select v-model="method" aria-label="Wizard weighting method" class="inspector-select w-full"><option value="surface">Surface smoothing</option><option value="distance">Distance blend</option></select></label><p v-for="error in errors" :key="error" class="text-ui-textSecondary leading-relaxed">{{ error }}</p></template>
            <p v-if="message" role="alert" class="text-ui-textSecondary leading-relaxed">{{ message }}</p>
          </aside>
        </div>
