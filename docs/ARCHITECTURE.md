...
Right sidebar (`RightSidebar.vue`): Tools / List / context (Object, UV, or Anim) / Mod / Mat / Tex. UV workspace orders Tex before Mat and Mod. Rig Inspect uses the same rail+sheet body as Modeling (Skel / Bone / Bind / Wts), not a separate wizard. Animate Inspect uses the same `inspector-head` + `UiSection` stack as Mesh Tools (clip, keys, pose, playback); it is not an Essentials tutorial card. The last tab is remembered per workspace (`layoutStore.setInspectorTab` / `restoreInspectorTab`) and restore lands on a tab visible in that workspace. Object vs vertex `selectMode` does not change the tab. Selection mode lives on the left toolbar (`1`/`2`/`3`/`4` / Tab). The Tools tab (`MeshToolsProps`) is a mode readout plus Subdivide params and unique operators (Faces / Edges / Vertices / Mesh); Extrude / Knife and the rest of the modal shelf stay on the left toolbar. Switching Vertex / Edge / Face still opens that Tools section. Sheets sit flush against the icon rail. Tab glyphs inherit `currentColor`; the active tab is a single accent pip. Sheet titles use the shared `inspector-head` class. In-sheet category strips use `inspector-seg`. UV/Paint workspace chrome (tabs, Smart UV, paint shade/opacity, layers header) uses the same family; canvas drawing (islands, seams, pixels) does not. Rig sheet chrome (starter chips, Bind/Weights toggles, auto-rigger, hierarchy popout) uses the same family; viewport bone colors stay. Animate sheet chrome uses the same family; timeline diamonds and onion-skin colors stay.
