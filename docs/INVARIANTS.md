...
- Confirm goes through `onCommit`; cancel restores the kernel snapshot then `onCancel`.
...
- Menu/`perform*` and modal kernel for the same verb (extrude, inset, bevel, …) must stay behaviorally aligned. Inset uses `InsetKernel` on both paths; **I** in object mode must not inset the whole mesh.
...
- Vertex/edge snap is a **rigid** offset for the whole selection (`SnapManager.findRigidSnapOffset`), on both `MoveOperator` and gizmo component drag.
